...
if(isset($_POST['sub']))
{
    $bc=$_POST['bc'];
    $pas=$_POST['pas'];
    $chk_user=mysqli_query($dbcon,"select * from cit_data where bar_code='$bc' and password='$pas'");
...
<head>
                                                            ...
                                                            {
                                                                $.colorbox({iframe:true, width:"70%", height:"470px", href: "medicine.php?sid="+a+"&pid="+b});
                                                              }
                                                            function shoopinfo(x)
                                                            {
                                                                $.colorbox({iframe:true, width:"70%", height:"470px", href: "showop.php?aid="+x});
                                                            }
                                                            function shoadmitinfo(x)
                                                            {
                                                                $.colorbox({iframe:true, width:"70%", height:"470px", href: "showadmit.php?aid="+x});
                                                            }
                                                            </script>
  <style type="text/css">
body{
                ...
                <div class="custom-services">
                    <div class="col-lg-12 col-md-12"><br />
                            ...
                            <div id="hlist">
                                ...
                                {
                                    $expire=  time() + (86400 * 30);
                                    setcookie("bar", $_GET['code'], $expire);
                                    if(isset($_COOKIE['bar']))
                                    {
                                        $bc=$_COOKIE['bar'];                                  
                                    $sel_pat=mysqli_query($dbcon,"select * from cit_data where bar_code='$bc'");
                                    $r_pat=mysqli_fetch_row($sel_pat);
                                    ?>
                                <div class="col-lg-6">
                         ...
                         <td><?php echo $r_pat[8] ?><br /><?php echo $r_pat[9] ?><br /><span class='fa fa-phone'></span><?php echo $r_pat[10] ?><hr />D.O.B : <?php echo $r_pat[13] ?><BR />Blood Group : <?php echo $r_pat[12] ?><br /><BR />
                             ...
                             <tr>
                                 <td colspan="2"><center>
                                 <a href="report1.php?report=1&t=1&code=<?php echo $_GET['code'] ?>"><span class="label label-warning">PREVIOUS TREATMENT HISTORY</span> </a>
                                 <a href="report1.php?report=1&t=2&code=<?php echo $_GET['code'] ?>"><span class="label label-primary">MEDICINE PURCHASE HISTORY</span> </a>
                                 <a href="report1.php?report=1&t=3&code=<?php echo $_GET['code'] ?>"><span class="label label-info">OLD LABORATORY REPORTS</span></a>
                             </center></td>                            
                         </tr>
                         <tr>
                             <td colspan="4"><center>
                    ...
                    <div class="col-lg-6 col-md-6">
                            ...
                            if(isset($_GET['t']))
                            {
                                if($_GET['t']==1)
                                {
                                   $sel_admit=mysqli_query($dbcon,"select * from new_patient where bar_code='$bc' order by id desc");
                                   if(mysqli_num_rows($sel_admit)>0)
                                   {
                                       ?>
                            <table class="table table-bordered table-condensed table-hover table-responsive table-striped">
                                <tr>
                                ...
                                <?php
                                $i=1;
                                while($r_admit=mysqli_fetch_row($sel_admit))
                                       {
                                           ?>
                                <tr>
                                    <td><?php echo $i ?></td>
                            ...
                            <?php                                       
                                ...
                                if($_GET['t']==2)
                                {
                                   echo"<h3>Medicine Purchase Details</h3>";
                                        $sel_pr=mysqli_query($dbcon,"select distinct dt from purchase_medicine where bar_code='$bc' order by id desc");
                                        if(mysqli_num_rows($sel_pr)>0)
                                        {
                                            ?>
                                    <script>
                                    function shosale1(x)
                                    ...
                                    <table class="table table-bordered table-condensed table-hover table-responsive table-striped">
                                        <?php
                                        $i=1;
                                        while($r_pr=mysqli_fetch_row($sel_pr))
                                        {
                                            ?>
                                        <tr>
                                            <td><div style="cursor: pointer; background-color: #006dcc; color: white;" onclick="shosale1('<?php echo $i ?>')"><b><?php echo $r_pr[0] ?></b></div>
                                                <div id="a<?php echo $i ?>" style="display: none;">
                                                    <?php
                                                    $sel_det=mysqli_query($dbcon,"select * from purchase_medicine where dt='$r_pr[0]' and bar_code='$bc'");
                                                    $j=1;
                                                    while($r_det=  mysqli_fetch_row($sel_det))
                                                    {
                                                        $sel_shop=mysqli_query($dbcon,"select * from org_data where unme='$r_det[1]'");
                                                        $r_shop=mysqli_fetch_row($sel_shop);
                                                        if($r_det[3]=="0")
                                                        {
                                ...
                                if($_GET['t']==3)
                                {
                                    ...
                                    <table class="table table-bordered table-condensed table-hover table-responsive table-striped">
                                        ...
                                        <?php
                                        $i=1;
                                        $sel_test=mysqli_query($dbcon,"select * from purchase_lab where barcode='$bc' order by id desc");
                                        while($r_test=mysqli_fetch_row($sel_test))
                                        {
                                            ?>
                                        <tr>
                                            <td><?php echo $i ?></td>
                                            <td>Test Done from <?php 
                                            $sel_store=mysqli_query($dbcon,"select * from org_data where unme='$r_test[1]'");
                                            $r_store=mysqli_fetch_row($sel_store);
                                        ...
                                        <tr id="<?php echo $i ?>" style="display: none;">
                                                                ...
                                                                <?php
                                                                $sel_l1=mysqli_query($dbcon,"select * from purchase_lab_test where pid='$r_test[0]'");
                                                               $j=1;
                                                                while($r_l1=mysqli_fetch_row($sel_l1))
                                                                {
                                                                    ?>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td colspan="2">
                                                                        <?php
                                                                        $sel_tst=mysqli_query($dbcon,"select * from labtest_data where id='$r_l1[5]'");
                                                                        $r_tst=mysqli_fetch_row($sel_tst);
                                                                        echo $r_tst[1];
                                                                        ?>
                                                                    </td>
                                                                    <td><center>
